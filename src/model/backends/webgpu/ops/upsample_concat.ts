...
export class UpsampleConcatWebGPU extends WebGPUOp {
  ...
  constructor(
    ...
    input: Tensor,   // decoder tensor at small spatial resolution
    skip: Tensor,    // encoder feature already at output resolution
    outH: number,
    outW: number,
  ) {
    super(device)
    const aGroups   = input.c / 4
    const bGroups   = skip.c / 4
    const outGroups = aGroups + bGroups

    this.output = makeOutputTensor(device, outH, outW, input.c + skip.c)
    ...
    this.bindGroup = device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 1, resource: { buffer: cast(skip).buffer } },
