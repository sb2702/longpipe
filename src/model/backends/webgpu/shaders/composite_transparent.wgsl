...
// background (premultiplied output) — whatever sits behind the canvas shows
// through wherever the matte is 0. Mirrors composite_solid.wgsl without the
// background color.
//
...
fn fs(in: VertexOut) -> @location(0) vec4<f32> {
    ...
    let fg = image[i].rgb;
    let a  = alpha[i].r;
    // Premultiplied subject over transparency: rgb·a, matte as alpha.
