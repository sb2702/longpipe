...
// 256×160, full encoder (tf_efficientnet_lite0, out_indices=(1,2,3,4)), standard decoder (128,64,32,16).
export class EfficientNetLiteMattingLarge {
  ...
  // Stage 1 — 16→24ch, e=6, k=3, s=2 first (skip feat1: s1b1.output, 24ch@stride-4)
  private readonly s1b0:  MBConv
  private readonly s1b1:  MBConv

  // Stage 2 — 24→40ch, e=6, k=3, s=2 first (skip feat2: s2b1.output, 40ch@stride-8)
  private readonly s2b0:  MBConv
  private readonly s2b1:  MBConv

  // Stage 3 — 40→80ch, e=6, k=3, s=2 first
  private readonly s3b0:  MBConv
  private readonly s3b1:  MBConv
  private readonly s3b2:  MBConv

  // Stage 4 — 80→112ch, e=6, k=3, s=1 (skip feat3: s4b2.output, 112ch@stride-16)
  private readonly s4b0:  MBConv
  private readonly s4b1:  MBConv
  private readonly s4b2:  MBConv

  // Stage 5 — 112→192ch, e=6, k=3, s=2 first
  private readonly s5b0:  MBConv
  private readonly s5b1:  MBConv
  private readonly s5b2:  MBConv
  private readonly s5b3:  MBConv

  // Stage 6 — 192→320ch, e=6, k=3, s=1 (bottleneck input: 320ch@stride-32)
  private readonly s6b0:  MBConv

  // Bottleneck — 320→128ch, conv1×1, relu6
  private readonly bottleneck: Op

  // Decoder
  private readonly dec0: DecoderBlock  // deep=128ch@stride-32, skip=feat3 112ch@stride-16 → 64ch
  private readonly dec1: DecoderBlock  // deep= 64ch@stride-16, skip=feat2  40ch@stride-8  → 32ch
  private readonly dec2: DecoderBlock  // deep= 32ch@stride-8,  skip=feat1  24ch@stride-4  → 16ch

  // Final — upsample ×2 + conv1×1 relu6, then reduce channels, then upsample ×2 + sigmoid
  private readonly finalUp: Op  // UpsampleConv1x1: stride-4 → stride-2, 16ch, relu6
  ...
  constructor(backend: Backend, input: Tensor, w: ModelWeights) {
    ...
    this.dec0 = new DecoderBlock(backend, this.bottleneck.output, this.s4b2.output, w.decoder.blocks[0], { outChannels: 64 })
    this.dec1 = new DecoderBlock(backend, this.dec0.output,       this.s2b1.output, w.decoder.blocks[1], { outChannels: 32 })
    this.dec2 = new DecoderBlock(backend, this.dec1.output,       this.s1b1.output, w.decoder.blocks[2], { outChannels: 16 })
    ...
    this.encoderTaps = [this.s1b1.output, this.s2b1.output, this.s4b2.output, this.s6b0.output]
