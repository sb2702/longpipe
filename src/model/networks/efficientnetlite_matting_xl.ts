...
// Full encoder (tf_efficientnet_lite0, out_indices=(1,2,3,4)), xl decoder (256,128,64,32).
// Identical encoder to Large; bottleneck + decoder channels are wider. Designed for 512×288 input.
export class EfficientNetLiteMattingXL {
  ...
  // Bottleneck — 320→256ch
  private readonly bottleneck: Op

  // Decoder
  private readonly dec0: DecoderBlock  // deep=256ch@stride-32, skip=feat3 112ch@stride-16 → 128ch
  private readonly dec1: DecoderBlock  // deep=128ch@stride-16, skip=feat2  40ch@stride-8  →  64ch
  private readonly dec2: DecoderBlock  // deep= 64ch@stride-8,  skip=feat1  24ch@stride-4  →  32ch
  ...
  constructor(backend: Backend, input: Tensor, w: ModelWeights) {
    ...
    this.dec0 = new DecoderBlock(backend, this.bottleneck.output, this.s4b2.output, w.decoder.blocks[0], { outChannels: 128 })
    this.dec1 = new DecoderBlock(backend, this.dec0.output,       this.s2b1.output, w.decoder.blocks[1], { outChannels:  64 })
    this.dec2 = new DecoderBlock(backend, this.dec1.output,       this.s1b1.output, w.decoder.blocks[2], { outChannels:  32 })
