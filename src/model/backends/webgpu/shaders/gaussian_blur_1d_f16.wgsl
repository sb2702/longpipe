...
// Separable 1D Gaussian blur — f16 storage variant. Weights and accumulator
// stay in f32 (the kernel sum needs f32 headroom for large radii / small
// sigmas), output written as f16.
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i: i32 = -R; i <= R; i = i + 1) {
        ...
        let v  = vec4<f32>(input_buf[sy * W * C + sx * C + c]);
        acc  = acc + w * v;
        wsum = wsum + w;
    }

    output_buf[y * W * C + x * C + c] = vec4<f16>(acc / wsum);
}
