...
// converges to the deadband and then holds exactly (prev is passed through, not
// re-lerped), so there's no slow drift. Tightening deadband/ease a lot would
// need revisiting.
...
fn main() {
    ...
    if (best < 0) { out_buf[0] = vec4<f16>(prev); return; }   // no subject → hold
    let subj = vec4<f32>(box_buf[u32(best)]);
