...
fn main() {
    ...
    if (chal < 0) {
        out_buf[0] = prev;
        out_buf[1] = prev_s;   // keep the memory: a one-frame detection miss must
    ...
    var pick = chal;
    if (inc >= 0 && chal_hs <= box_buf[u32(inc)].z * SWITCH_RATIO) { pick = inc; }
    let subj = box_buf[u32(pick)];
    out_buf[1] = vec4<f32>(subj.x, subj.y, subj.z, 1.0);
